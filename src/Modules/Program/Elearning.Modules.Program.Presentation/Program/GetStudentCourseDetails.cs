...
using Elearning.Modules.Program.Application.Program.GetStudentCourseDetails;
using MediatR;
...
internal sealed class GetStudentCourseDetails : IEndpoint
{
  public void MapEndpoint(IEndpointRouteBuilder app)
  {
    app.MapGet("student/courses-detail/{courseId}", async (
        Guid courseId,
        ...
        HttpContext httpContext) =>
    {
      ...
      var result = await sender.Send(new GetStudentCourseDetailsQuery(courseId, effectiveStudentId));

      return result.Match(
          data => Results.Ok(new
          {
            status = StatusCodes.Status200OK,
            message = "Get student course details successfully",
            data
          }),
          ApiResults.Problem
      );
