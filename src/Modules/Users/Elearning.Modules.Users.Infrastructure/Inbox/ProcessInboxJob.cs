...
using Elearning.Common.Infrastructure.Serialization;
using Elearning.Common.Infrastructure;
namespace Elearning.Modules.Users.Infrastructure.Inbox;
           ...
           ) : IJob
{
  ...
  public async Task Execute(IJobExecutionContext context)
  {
    ...
    foreach (InboxMessageResponse inboxMessage in inboxMessages)
    {
      ...
      try
      {
        IIntegrationEvent integrationEvent = JsonConvert.DeserializeObject<IIntegrationEvent>(inboxMessage.Content, SerializerSettings.Instance);
      ...
      catch (Exception caughtException)
      {
        logger.LogError(
          caughtException,
          "Module {Module} - Error while processing inbox message {InboxMessageId}",
          ModuleName,
          inboxMessage.Id
        );
