...
internal sealed class IdempotentIntegrationEventHandler<TIntegrationEvent>(
  IIntegrationEventHandler<IIntegrationEvent> decorated,
    ...
    where TIntegrationEvent : IIntegrationEvent
{
  ...
  )
  {
    const string sql =
            ...
            SELECT EXISTS(
                SELECT 1
                FROM users.table_inbox_message_consumers
      ...
      InboxMessageConsumer inboxMessageConsumer)
  {
    const string sql =
        """
            INSERT INTO users.table_inbox_message_consumers(inbox_message_id, name)
